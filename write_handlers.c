																...
																for (i = 0; i < field_width - 1; i++)
																		...
																		if (format_flags & F_MINUS)
																						return (write(1, &buffer[0], 1) +
																													write(1, &buffer[BUFF_MODIFIER - i - 1], field_width - 1));
																				else
																								return (write(1, &buffer[BUFF_MODIFIER - i - 1], field_width - 1) +
																															write(1, &buffer[0], 1));
																					}
				...
				UNUSED(modifier);
					...
					if ((format_flags & F_ZERO) && !(format_flags & F_MINUS))
						...
						if (is_negative)
									...
									return (write_num(ind, buffer, format_flags, field_width, precision,
...
int write_num(int ind, char buffer[],
			int format_flags, int field_width, int prec,
...
int write_pointer(char buffer[], int ind, int length,
			int field_width, int format_flags, char padd, char extra_c, int padd_start)
{
		int i;
